...
/**
 * This file is part of cytoscape.js 2.0.0beta2-github-snapshot-2012.11.21-10.43.22.
 * 
...
}}ac=true;T=true;var an=E.renderer().projectMouse(ak);X[0]=an[0];X[1]=an[1];if(ar.button==0&&ar.target==E.renderer().canvas&&ae==undefined&&N==undefined&&!aq){X[4]=1}if(ar.button==0){if(ae!=undefined&&ae.grabbable()){j=true;c=[];if(ae.selected()){I=true;for(var at=0;at<al.length;at++){if(al[at].selected()&&al[at].grabbable()){al[at]._private.rscratch.dragStartX=al[at]._private.position.x;al[at]._private.rscratch.dragStartY=al[at]._private.position.y;c.push(al[at]);al[at]._private.grabbed=true;al[at].trigger(new aa.Event(ak,{type:"grab"}))
}}}else{if(ae.grabbable()){I=false;m=ae;m._private.rscratch.dragStartX=m._private.position.x;m._private.rscratch.dragStartY=m._private.position.y;c.push(m);m._private.rscratch.layer2=true;m._private.grabbed=true;m.trigger(new aa.Event(ak,{type:"grab"}))}}l=U.findEdges(c);for(var ap=0;ap<l.length;ap++){}v=false;if(aq){ae.trigger(new aa.Event(ak,{type:"touchstart"}))}else{ae.trigger(new aa.Event(ak,{type:"mousedown"}))}C=ae}else{if(N!=undefined){if(aq){N.trigger(new aa.Event(ak,{type:"touchstart"}))
}else{N.trigger(new aa.Event(ak,{type:"mousedown"}))}a=N}else{if(aq){E.trigger(new aa.Event(ak,{type:"touchstart"}))}else{E.trigger(new aa.Event(ak,{type:"mousedown"}))}ac=true}}}E.renderer().redraw()};af.prototype.mouseOverHandler=function(ak){e=true;S=true};af.prototype.mouseOutHandler=function(ak){ab=false;S=false;E.trigger(new aa.Event(ak,{type:"mouseout"}));s=undefined};af.prototype.touchStartHandler=function(ak){};af.prototype.touchStartHandler=function(ak){};af.prototype.documentMouseMoveHandler=function(al){var am=false;
var ak=al;if(al.touches){am=true;ak=al.touches[0]}};af.prototype.mouseMoveHandler=function(aC){S=true;Q=setTimeout(function(){Q=null},1000/100);var az=aC;var aq=false;if(aC.touches){aC.preventDefault();aq=true;if(aC.touches.length>=2){var aA=[U.canvas.parentElement.offsetLeft,-U.canvas.parentElement.offsetTop];r[0]=az.touches[0].clientX+aA[0];r[1]=az.touches[0].clientY+aA[1];V[0]=az.touches[1].clientX+aA[0];V[1]=az.touches[1].clientY+aA[1];B=Math.sqrt(Math.pow(O[0]-k[0],2)+Math.pow(O[1]-k[1],2));
var au=[r[0]-k[0],r[1]-k[1]];var at=[V[0]-O[0],V[1]-O[1]];var ak=[(au[0]+at[0])/2,(at[1]+at[1])/2];var ao=B/n;if(ao>1){ao=(ao-1)*1.5+1}else{ao=1-(1-ao)*1.5}G=true;E.panBy({x:ak[0],y:ak[1]});E.zoom({level:E.zoom()*ao,position:{x:(r[0]+V[0])/2,y:(r[1]+V[1])/2}});k[0]=r[0];k[1]=r[1];O[0]=V[0];O[1]=V[1];n=B;return}aC=aC.touches[0];aC.button=0}var ay=az;var an=U.mouseMoveHelper.dragHandler;var al=U.mouseMoveHelper.hoverHandler;var am=E.edges();var aw=E.nodes();Y=undefined;if(ah){an(aC)}var av=E.renderer().projectMouse(aC);
ai=aC.screenX;if(s!==undefined&&Math.abs(s-ai)>1){ab=true}s=ai;X[2]=av[0];X[3]=av[1];if(!X[4]){al(aw,am,aC)}C=undefined;a=undefined;T=false;if(ae!=undefined){if(E.renderer().canvas.style.cursor!=ae._private.style.cursor.value){E.renderer().canvas.style.cursor=ae._private.style.cursor.value}if(u!==ae){if(u!==undefined){if(aq){}else{u.trigger(new aa.Event(az,{type:"mouseout"}))}}u=ae;var ar=ae.grabbed();if(aq&&!ar){}else{if(!aq&&!ar){ae.trigger(new aa.Event(az,{type:"mouseover"}))}}}else{if(aq){ae.trigger(new aa.Event(az,{type:"touchmove"}))
}else{ae.trigger(new aa.Event(az,{type:"mousemove"}))}}}else{if(N!=undefined){if(E.renderer().canvas.style.cursor!=N._private.style.cursor.value){E.renderer().canvas.style.cursor=N._private.style.cursor.value}if(ad!==N){if(ad!==undefined){if(aq){}else{ad.trigger(new aa.Event(az,{type:"mouseout"}))}}ad=N;var ax=N.grabbed();if(aq&&!ax){}else{if(!aq&&!ax){N.trigger(new aa.Event(az,{type:"mouseover"}))}}}else{if(aq){N.trigger(new aa.Event(az,{type:"touchmove"}))}else{N.trigger(new aa.Event(az,{type:"mousemove"}))
}}}else{if(!ae&&!N&&E.renderer().canvas.style.cursor!="default"){E.renderer().canvas.style.cursor="default"}if(ad!==undefined){if(aq){}else{ad.trigger(new aa.Event(az,{type:"mouseout"}))}ad=undefined}if(u!==undefined){if(aq){}else{u.trigger(new aa.Event(az,{type:"mouseout"}))}u=undefined}if(e){if(aq){}else{E.trigger(new aa.Event(az,{type:"mouseover"}))}S=true;e=false}else{if(S){if(aq){E.trigger(new aa.Event(az,{type:"touchmove"}))}else{E.trigger(new aa.Event(az,{type:"mousemove"}))}}}}}if(j){if(!v){for(var aB=0;
aB<c.length;aB++){c[aB]._private.rscratch.layer2=true}for(var aB=0;aB<l.length;aB++){l[aB]._private.rscratch.layer2=true}U.canvasNeedsRedraw[4]=true;U.redrawReason[4].push("nodes being dragged, moved to drag layer");U.canvasNeedsRedraw[2]=true;U.redrawReason[2].push("nodes being dragged, moved to drag layer");v=true}for(var ap=0;ap<aw.length;ap++){if((I&&aw[ap].selected())||(!I&&aw[ap]==m)){if(!aw[ap]._private.locked&&aw[ap]._private.grabbable){aw[ap]._private.position.x=aw[ap]._private.rscratch.dragStartX+(X[2]-X[0]);
aw[ap]._private.position.y=aw[ap]._private.rscratch.dragStartY+(X[3]-X[1]);aw[ap].trigger(new aa.Event(az,{type:"drag"}));aw[ap].trigger(new aa.Event(az,{type:"position"}))}}}U.canvasNeedsRedraw[2]=true;U.redrawReason[2].push("nodes being dragged")}if(X[4]){U.canvasNeedsRedraw[0]=true;U.redrawReason[0].push("selection boxed moved")}if(ah||j||X[4]){E.renderer().redraw()}};af.prototype.mouseUpHandler=function(aA){var av=undefined;if(aA.changedTouches){aA.preventDefault();av=aA;aA=aA.changedTouches[0];
aA.button=0;X[2]=U.projectMouse(aA)[0];X[3]=U.projectMouse(aA)[1]}var az=aA;var am=E.edges();var ax=E.nodes();var au=j&&(Math.abs(X[2]-X[0])+Math.abs(X[3]-X[1])>1);for(var aC=0;aC<c.length;aC++){c[aC]._private.rscratch.layer2=false;c[aC]._private.grabbed=false;c[aC].trigger(new aa.Event(aA,{type:"free"}))}c=[];for(var aC=0;aC<l.length;aC++){l[aC]._private.rscratch.layer2=false}var aB=undefined;if(ae!==undefined){aB=ae}else{if(N!==undefined){aB=N}}Y=undefined;var at=undefined;if(av){at="touchend"}else{at="mouseup"
}if(aB!=undefined){aB.trigger(new aa.Event(aA,{type:at}));if(aB===C||aB===a){aB.trigger(new aa.Event(aA,{type:"click"}))}}else{E.trigger(at);if(T){E.trigger(new aa.Event(aA,{type:"click"}))}}if(!w&&!au){var aq=E.collection();for(var ao=0;ao<ax.length;ao++){ax[ao]._private.rscratch.selected=false;if(ax[ao]._private.selected){aq=aq.add(ax[ao])}}for(var ao=0;ao<am.length;ao++){am[ao]._private.rscratch.selected=false;if(am[ao]._private.selected){aq=aq.add(am[ao])}}if(aq.length>0){aq.unselect()}}if(X[4]==1&&!j&&Math.abs(X[2]-X[0])+Math.abs(X[3]-X[1])>2){var ar=2;
var ak;var aw;var aE=E.collection();for(var ao=0;ao<am.length;ao++){ak=am[ao]._private.selected;var an;var al=am[ao]._private.rscratch;if(am[ao]._private.rscratch.isStraightEdge){an=aa.math.boxInBezierVicinity(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,(am[ao]._private.rscratch.startX+am[ao]._private.rscratch.endX)/2,(am[ao]._private.rscratch.startY+am[ao]._private.rscratch.endY)/2,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)}else{if(am[ao]._private.rscratch.isSelfEdge){an=aa.math.boxInBezierVicinity(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,am[ao]._private.rscratch.cp2ax,am[ao]._private.rscratch.cp2ay,am[ao]._private.rscratch.selfEdgeMidX,am[ao]._private.rscratch.selfEdgeMidY,ar);
if(an==0){an=aa.math.boxInBezierVicinity(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.selfEdgeMidX,am[ao]._private.rscratch.selfEdgeMidY,am[ao]._private.rscratch.cp2cx,am[ao]._private.rscratch.cp2cy,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)}}else{an=aa.math.boxInBezierVicinity(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,am[ao]._private.rscratch.cp2x,am[ao]._private.rscratch.cp2y,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)
}}if(an==2){aw=true}else{if(an==1){if(am[ao]._private.rscratch.isSelfEdge){aw=aa.math.checkBezierCrossesBox(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,am[ao]._private.rscratch.cp2ax,am[ao]._private.rscratch.cp2ay,am[ao]._private.rscratch.selfEdgeMidX,am[ao]._private.rscratch.selfEdgeMidY,ar);if(!aw){aw=aa.math.checkBezierCrossesBox(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.selfEdgeMidX,am[ao]._private.rscratch.selfEdgeMidY,am[ao]._private.rscratch.cp2cx,am[ao]._private.rscratch.cp2cy,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)
}}else{if(am[ao]._private.rscratch.isStraightEdge){aw=aa.math.checkStraightEdgeCrossesBox(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)}else{aw=aa.math.checkBezierCrossesBox(X[0],X[1],X[2],X[3],am[ao]._private.rscratch.startX,am[ao]._private.rscratch.startY,am[ao]._private.rscratch.cp2x,am[ao]._private.rscratch.cp2y,am[ao]._private.rscratch.endX,am[ao]._private.rscratch.endY,ar)}}}else{aw=false}}if(aw&&!ak){aE=aE.add(am[ao])
}else{if(!aw&&ak){}}}var aI=Math.min(X[0],X[2]);var aH=Math.min(X[1],X[3]);var aG=Math.max(X[0],X[2]);var aF=Math.max(X[1],X[3]);var aD,aw;var ay,ap;for(var ao=0;ao<ax.length;ao++){aD=ax[ao]._private.selected;ay=ax[ao].position();if(ay.x>aI&&ay.x<aG&&ay.y>aH&&ay.y<aF){aw=true;ax[ao]._private.rscratch.selected=true}else{if(t[ax[ao]._private.style.shape.value].intersectBox(aI,aH,aG,aF,ax[ao]._private.style["border-width"].value/2,ax[ao]._private.style.width.value,ax[ao]._private.style.height.value,ay.x,ay.y)){aw=true;
...
this.system=m;if(k.liveUpdate&&k.fit){j.reset()}var t=250;var g;var o=false;var f=+new Date;var r={init:function(z){},redraw:function(){var A=m.energy();if(k.stableEnergy!=null&&A!=null&&A.n>0&&k.stableEnergy(A)){m.stop();return}clearTimeout(g);g=setTimeout(w,t);var B=[];m.eachNode(function(H,C){var G=H.name;var E=H.data;var D=E.element;if(D==null){return}var F=D._private.position;if(!D.locked()&&!D.grabbed()){F.x=C.x;F.y=C.y;B.push(D)}});var z=(+new Date-f)>=16;if(k.liveUpdate&&B.length>0&&z){new a.Collection(j,B).rtrigger("position");
