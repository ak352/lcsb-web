...
outf = open("human_gene_loci.dat", "w")
c.execute("select distinct(gene_name), chr, start, end from " + ref + "_ref order by chr asc")
#print "var " + ref + "_chrgene_info = [ "
chrs = c.fetchall()
for cc in chrs:
	#chr = cc[0]
	if (pomo_util.is_number(cc[0])):
		continue
	outf.write(cc[0] + ":chr" + " ".join(cc[1:]) + "\n")
	#reft = ref + "_ref"
